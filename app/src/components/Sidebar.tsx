    ...
    return (
        <aside className="flex w-72 shrink-0 flex-col border-r border-surface-card-border bg-surface-page-deep">
            <div className="space-y-3 border-b border-surface-card-border p-4">
                ...
                <span
                    className="inline-flex items-center gap-1.5 rounded-full bg-success-bg px-2.5 py-1 text-xs text-success">
            ...
            <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-3">
                <p className="flex justify-between w-1/1 px-3 pb-1 mb-5 text-xs font-medium tracking-wide text-ink-muted relative">
                    PROJETOS & AMBIENTES

                    {!creatingProject && (
                        <button
                            onClick={onStartCreatingProject}
                            className="flex items-center gap-2 rounded-full px-0.5 py-0.5 text-left text-sm bg-surface-card-border
                        text-ink-muted hover:bg-surface-card hover:text-ink-secondary absolute right-0"
                        >
                            <PlusIcon className="h-4.5 w-4.5 text-ink-muted rounded-full "/>
                        </button>
                    )}
                ...
                {creatingProject && (
                    <form onSubmit={onCreateProject} className="space-y-2 px-1 py-2">
                        <input
                            ...
                            className="w-full rounded-md border border-none px-2 py-1.5
                            text-sm text-ink-primary placeholder:text-ink-muted outline-none"
                        />
                        <div className="flex gap-2 float-right">
                            <button
                                type="button"
                                onClick={onCancelCreatingProject}
                                className="rounded-md border border-surface-card-border px-2 py-1 text-xs
